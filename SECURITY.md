...
| **SSRF via the API base URL** | The base is operator-configurable, so it is validated and its host resolved before connecting. Loopback, RFC1918, link-local, reserved, multicast, unspecified, carrier-grade NAT and the IPv4-mapped, 6to4 and NAT64 spellings of all of them are refused, and so is anything else that is not globally routable unicast. An address the guard cannot parse is refused rather than skipped: skipping it meant a host resolving only to such an address was connected to with nothing checked. Redirects are not followed at all — an API endpoint that redirects is either misconfigured or hostile |
...
| **Secret leakage through a log line** | Every log record passes a redacting filter using the same credential patterns the intake is scrubbed with, and tracebacks are redacted explicitly because they are formatted separately and never pass through `getMessage()` — which is exactly where a raw value tends to surface, since a traceback carries the arguments that caused the failure. `httpx`'s URL logging is suppressed |
...
| **Redirection or stalling of any other file** | Every file this tool opens — the config, the intake form, an attached design document, the review it writes and the baseline — goes through a descriptor with `O_NOFOLLOW`, and every read adds `O_NONBLOCK`. The second one matters as much as the first: opening a FIFO blocks *inside* `os.open`, before the regular-file check that exists to reject it, so a named pipe left at any of those paths hung the command indefinitely with no error and nothing in the log. The file type and size come from `fstat` on the descriptor actually read, not from a `stat()` of a name that may since have become something else |
| **A review readable by anyone on the host** | Written `0600` through that same descriptor, with the mode set by `fchmod` rather than `chmod` on the path. A review names the system, the team, the owner and every gap; and a `chmod` by path could be aimed at somebody else's file to lock its owner out of it |
| **Autolinking** — an answer needs no Markdown syntax at all to put a live link in a review | A URL-shaped run in untrusted text is rendered inside a code span, where no CommonMark implementation autolinks. Escaping cannot address this: the autolinker matches the shape of the text rather than a metacharacter. Prose written in this repository is still not escaped, so `gap(s)` stays `gap(s)` |
